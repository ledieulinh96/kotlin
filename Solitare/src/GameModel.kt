object GameModel {
    ...
    fun resetGame() {
        ...
        tableauPiles.forEachIndexed { i, tableauPile ->
            val cardsInPile: MutableList<Card> = Array(i + 1, { desk.drawCard() }).toMutableList()
            //{} tu fill theo size luon giong java chang :3
            tableauPiles[i] = TableauPile(cardsInPile)
        }
    }

    fun onDeskTap() {
        if (desk.cardsInDesk.size > 0) {
            val card = desk.drawCard()
            card.faceUp = true
            wastePile.add(card)
        } else {
            desk.cardsInDesk = wastePile.toMutableList()
            wastePile.clear()
        }
    }
